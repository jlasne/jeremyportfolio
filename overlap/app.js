...
function renderSteps(){
  ...
  if(el){
    el.textContent="4";
    el.className="stepn"+(now===4?" on":" wait");
  }
}
/* Step 2 is a thing you get, so show it once you have it. */
function renderLink(){
  var c=$("#linkCard"), g=$("#getLink");
  if(!c) return;
  if(!inMeeting()){
    c.innerHTML='<div class="empty"><b>No link yet.</b>'+
      "Name the meeting first, then get its link.</div>";
    g.style.display=""; g.disabled=!(LIVE&&signedIn()&&ready()&&S.title.trim());
    $("#linkSum").textContent="not yet";
    $("#linkFoot").textContent=S.title.trim()
      ? "The link is how everyone else gets in."
      : "Name it in step 1 and this wakes up.";
    return;
  }
...
function renderDock(){
  ...
  if(LIVE && !inMeeting()){
    ...
    h.textContent=S.title.trim()
      ? "Get the link in step 2 first."
      : "Name it in step 1, then get the link in step 2.";
    return;
  }
  if(total<2){
    b.disabled=true;
    h.textContent="Nobody has answered yet. Invite them in step 3.";
    return;
  }
  if(!t){
    b.disabled=true;
...
$("#getLink").addEventListener("click",function(){
  ...
  if(!S.title.trim()) return toast("Name the meeting in step 1 first");
  var g=$("#getLink"), was=g.textContent;
  g.disabled=true; g.textContent="…";
  createMeeting().catch(function(e){ toast(e.message); })
    .then(function(){ g.disabled=false; g.textContent=was; renderLink(); renderSteps(); });
});
...
$("#titleInput").addEventListener("input",function(){
  ...
  renderRail(); renderLink(); renderSteps(); renderDock();
...
$("#primary").addEventListener("click",function(){
  if(LIVE && !signedIn()) return location.assign(basePath()+"/login/");
  if(LIVE && !ready()) return openSheet(true);
  if(LIVE && !inMeeting()) return toast("Get the link in step 2 first");
