...
function renderPeople(){
  ...
  html+='<div class="row tap" data-share="1"><div class="plus">↗</div>'+
        '<div class="grow"><div class="t">Share the link</div>'+
        '<div class="s">They answer for themselves</div></div><div class="chev"></div></div>';
  $("#peopleList").innerHTML=html;

  var n=guests().length, total=S.people.length;
  $("#peopleCount").textContent=foldOpen("peopleSec")
    ? (n===total ? total+(total===1?" person":" people") : n+" of "+total)
    : S.people.map(pname).slice(0,3).join(", ")+(total>3?" +"+(total-3):"");
  $("#guestFoot").textContent=!n
    ? "Nobody is ticked, so there is nothing to look for."
    : total===1
      ? "Just you so far. Share the link and their hours land here."
      : "Ticked people count towards the overlap, and get the invitation.";
    ...
    $("#bestList").innerHTML=rows.map(function(x){
      ...
      return '<button class="slot'+(here===x.ts?" on":"")+'" data-pick="'+x.ts+'">'+
        ...
        '<span class="w1">'+fmtDate(x.ts,tz)+'<span class="w1s"> · '+
          (x.manual?((MEET&&MEET.startsAt===x.ts)?"booked":"your pick"):(all?"everyone free":x.n+" of "+total+" free"))+"</span></span>"+
...
function createMeeting(){
  ...
  return cx("meet.create",{token:token(),title:S.title.trim(),durationMin:S.dur,tz:LOCAL_TZ})
    ...
    .then(function(){ afterChange(); haptic(); toast("Link ready — share it"); openShare(); });
}
...
function renderRail(){
  ...
  $("#railFoot").innerHTML=
    ...
    (inMeeting()
      ?'<button class="rail-team" data-share="1"><span class="plus">↗</span>'+
       '<span class="rail-tx"><b>Share</b><span>One link, they do the rest</span></span></button>':"")+
    (signedIn()
...
function renderDock(){
  ...
  $("#shareBtn").style.display=inMeeting()?"":"none";
  ...
  if(LIVE && !inMeeting()){
    b.textContent="Create meeting"; b.disabled=false;
  ...
  b.disabled=!t;
  if(!t){
    h.textContent=total<2
      ? "Just you so far — share the link"
      : "No hour works yet — widen someone’s hours or look at next week";
    return;
  }
...
document.addEventListener("click",function(ev){
  ...
  if((el=up("data-dur"))){
    S.dur=+el.getAttribute("data-dur"); afterChange(); renameMeeting(); return;
  }
  if((el=up("data-pick"))){
...
$("#titleInput").addEventListener("input",function(){
  S.title=this.value; save(); renderRail();
...
renderAll();
var landOn=(typeof OVERLAP_STEP==="number"?OVERLAP_STEP:stepFromPath());
  ...
  refreshMe().then(function(){
    ...
    if(joinCode){
      ...
      return joinMeeting(code).then(function(){
        toast("You’re in — " + (MEET?MEET.title:"the meeting"));
