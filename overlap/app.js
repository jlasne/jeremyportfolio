...
/* The page is static and the backend is deployed by hand, so for a while the
   ...
   http.ts answers a rejected argument with a sentence rather than the
   validator's own words, because the arguments carry a session token — so
   that sentence is what has to be matched here. */
var LATELY=["startHour2","endHour2","cal","calFrom","calTo"];
function cxNew(op,args){
  return cx(op,args).catch(function(e){
    if(!/newer than the server|validator|ArgumentValidation/i.test(e.message||"")) throw e;
    var lean={},k;
