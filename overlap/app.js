...
function renderPeople(){
  ...
  html+=PEEP.length
    ? '<div class="row tap" data-peeps="1"><div class="plus">+</div>'+
      '<div class="grow"><div class="t">Invite someone you have met</div>'+
      '<div class="s">'+PEEP.length+' in your people</div></div><div class="chev"></div></div>'
    : '<div class="row tap" data-share="1"><div class="plus">+</div>'+
      '<div class="grow"><div class="t">Send them the link</div>'+
      '<div class="s">They answer for themselves</div></div><div class="chev"></div></div>';
  ...
  $("#guestFoot").textContent=!n
    ...
    : total===1
      ? (inMeeting()?"Waiting. Whoever opens the link turns up here."
                    :"Get the link first, then send it to them.")
  ...
  $("#meetFoot").textContent=!LIVE
    ...
    : MEET ? "Everyone who opens the link sees this name."
           : "Name it, then press Create meeting to get a link.";
...
function stepDone(n){
  if(n===1) return !!S.title.trim();
  if(n===2) return inMeeting();
  if(n===3) return guests().length>1;
  if(n===4) return !!(MEET&&MEET.startsAt);
  return false;
}
function stepNow(){
  for(var n=1;n<=4;n++) if(!stepDone(n)) return n;
  return 4;
}
function renderSteps(){
  ...
  el=$("#dockStep");
  if(el) el.className="stepn"+(now===4?" on":" wait");
}
...
function renderLink(){
  ...
  if(!c) return;
  if(!inMeeting()){
    c.innerHTML='<div class="empty"><b>No link yet.</b>'+
      'Name the meeting, then press the button below.</div>';
    $("#linkSum").textContent="not yet";
    $("#linkFoot").textContent="The link is how everyone else gets in.";
    return;
  }
  c.innerHTML='<div class="linkbox">'+esc(shareUrl())+"</div>"+
    '<div class="row tap" data-share="1"><div class="plus">↗</div>'+
    '<div class="grow"><div class="t">Copy and send it</div></div><div class="chev"></div></div>';
  $("#linkSum").textContent="ready";
  $("#linkFoot").textContent="Anyone who opens it signs in and answers for themselves.";
}
function renderRail(){
...
function renderAll(){
  renderAcct(); renderRail(); renderPeople(); renderLink(); renderDays(); renderWTB();
...
function renderDock(){
  ...
  if(LIVE && !inMeeting()){
    b.textContent="Get the link"; b.disabled=false;
    h.textContent=S.title.trim()
      ? "Makes the link you send round."
      : "Name it first. That is what everyone else will see.";
    return;
  }
  b.textContent=MEET&&MEET.startsAt?"Open in Google Calendar":"Add to Google Calendar";
  b.disabled=!t;
  if(total<2){
    /* an hour that works for one person is not news. Say what is missing,
       but leave the button alive: a solo hold is nobody else's business. */
    h.textContent="Waiting for them. Send the link, or invite someone you have met.";
    return;
  }
  if(!t){
...
$("#shareCopy").addEventListener("click",function(){ copyText(shareUrl(),"Link copied"); });
$("#shareCopyBig").addEventListener("click",function(){ copyText(shareUrl(),"Link copied"); });
...
$("#titleInput").addEventListener("input",function(){
  ...
  renderRail(); renderSteps(); renderDock();
...
$("#primary").addEventListener("click",function(){
  ...
  if(LIVE && !inMeeting()){
    ...
    var b=$("#primary"); b.disabled=true; b.textContent="…";
    createMeeting().catch(function(e){ toast(e.message); })
      .then(function(){ b.disabled=false; renderDock(); });
    return;
  }
